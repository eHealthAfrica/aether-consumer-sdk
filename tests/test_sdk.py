...
# mock consumer
@pytest.mark.unit
def test_schema_validate(mocked_consumer):
    c = mocked_consumer
    job = {'a': 1}
    assert(c.validate(job, _type='job') is False)
    assert(c.validate(job, _type='resource') is False)
    errors = c.validate(job, _type='resource', verbose=True).get('validation_errors')
    assert(len(errors) == 2)
